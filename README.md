...
- **Inspired by “SimpleChat（space.bilibili.com/1447858895）“**
...
- **个性昵称**：每个用户可设置显示昵称，**修改后自动同步所有历史消息**（方案A），告别显示混乱。  
  **Custom display name** – Each user can set a nickname, and **all historical messages are automatically updated** (Plan A) after change, eliminating display inconsistency.
...
- **图片分享**：支持点击附件按钮上传图片，自动压缩（>200KB 压缩至 70% 质量），并生成预览，点击图片可放大/下载。  
  **Image sharing** – Upload images via the attachment button with automatic compression ( >200KB compressed to 70% quality), preview generation, and click-to-enlarge/download.
...
- **新消息通知**：页面后台时自动增加未读计数，并支持浏览器桌面通知（需授权）。  
  **New message notifications** – Auto-increments unread count when page is in background, with desktop notification support (requires permission).

- **在线用户显示** / **PWA支持** / 更多内容等你来探寻，提交你的PUSH！  
  **Online user display** / **PWA support** / More features to explore – submit your PR!
...
- Fork 本仓库，修改 `index.html` 中的 `SUPABASE_URL` 和 `SUPABASE_ANON_KEY` 为你的值（或使用环境变量）。  
  Fork this repo, replace `SUPABASE_URL` and `SUPABASE_ANON_KEY` in `index.html` with your own (or use environment variables).
...
Replace hardcoded values with environment variables (e.g., in Vercel):

```javascript
// 在 index.html 中替换硬编码为：
// Replace hardcoded values in index.html with:
...
```
minichat.astras/
├── index.html          # 完整应用（HTML + CSS + JS）| Full application (HTML + CSS + JS)
├── assets/             # 图标资源（logo.svg, Inspired.svg, basied.svg）| Icon assets
├── README.md
└── LICENSE
```
...
- 用户修改昵称时，会同时更新 `profiles` 表并 **批量更新** 该用户所有历史消息的 `sender_name` 字段，确保所有旧消息显示新昵称。  
  When a user changes their nickname, the `profiles` table is updated and **all historical messages** from that user have their `sender_name` **bulk‑updated**, ensuring old messages show the new nickname.
...
- 撤回后，所有在线客户端通过 Realtime 订阅接收到 `UPDATE` 事件，自动将消息内容改为“（已撤回）”，并移除撤回按钮。  
  After recall, all online clients receive an `UPDATE` event via Realtime subscription, automatically changing the message content to “(recalled)” and removing the recall button.

### 图片上传与压缩 | Image Upload & Compression
- 支持点击附件按钮选择图片，自动压缩（长边限制 1080px，质量 70%），上传至 Supabase Storage。  
  Supports selecting images via the attachment button, with automatic compression (long side limited to 1080px, quality 70%), then upload to Supabase Storage.
- 上传成功后插入带有 Markdown 图片语法 `![image](url)` 的消息，前端渲染为可点击预览的图片。  
  On successful upload, inserts a message with Markdown image syntax `![image](url)`, rendered as a clickable preview.
